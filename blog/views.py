...
from .forms import ContactForm, ForgotPassword, LoginForm, PostForm, RegisterForm, ResetPasswordForm
...
def details(request, post_id):
    # giving permission to view posts to the users
    if request.user and not request.user.has_perm("blog.view_post"):
        messages.error(request, "You have no permission to view posts")
...
def new_url_view(request):
    return HttpResponse("You are at the new url page !!!")
...
def register(request):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save(commit=False) # It will bring the data from the POST without committing in DB
...
def forgot_password(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #link genereation to reset password
...
@login_required
@permission_required('blog.add_post', raise_exception=404)
...
@login_required
@permission_required('blog.can_post', raise_exception=404)
